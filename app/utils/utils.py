...
def validate_model(model,
                   ...
                   directory='dataset/evaluation_2'):
...
def image_transf_cl(im):
    image_size = 32
    
    image = Image.open(im)
    image_gray = T.Grayscale(num_output_channels=1)(image)    
    image_resized = T.Resize(image_size)(image_gray)
...
def image_transf(im_in, im_out):
    image_size = 32
    
    image_in = Image.open(im_in)
    image_out = Image.open(im_out)
 
    image_in_gray = T.Grayscale(num_output_channels=1)(image_in)
    image_out_gray = T.Grayscale(num_output_channels=1)(image_out)
    
    image_in_resized = T.Resize(image_size)(image_in_gray)
    image_out_resized = T.Resize(image_size)(image_out_gray)
    
    image_in_tensor = T.ToTensor()(image_in_resized)
    image_out_tensor = T.ToTensor()(image_out_resized)

    image_in_transformed = image_in_tensor.repeat(3, 1, 1)
    image_out_transformed = image_out_tensor.repeat(3, 1, 1)
    
 
    return image_in_transformed, image_out_transformed

...
class AttnBlock(nn.Module):
  ...
  def __init__(self, channels):
    super().__init__()
    self.GroupNorm_0 = nn.GroupNorm(num_groups=4, num_channels=channels, eps=1e-6)
...
class ResnetBlockDDPM(nn.Module):
  ...
  def __init__(self, act, in_ch, out_ch=None, temb_dim=None, conv_shortcut=False, dropout=0.1):
    ...
    if out_ch is None:
      out_ch = in_ch
    self.GroupNorm_0 = nn.GroupNorm(num_groups=4, num_channels=in_ch, eps=1e-6)
    self.act = act
    ...
    self.GroupNorm_1 = nn.GroupNorm(num_groups=4, num_channels=out_ch, eps=1e-6)

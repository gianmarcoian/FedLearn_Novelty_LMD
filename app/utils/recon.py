...
from .recon_utils import get_model_ema, get_sde, get_model_ema_current_model
...
class Detector(object):
    def __init__(self, config, model_path, state): 
...
def run_recon_pretrained_model(image_in,image_out, config):
    
    detector = Detector(config, "/ddpm_models/732K/checkpoint_10.pth", None)
    ...
    with torch.no_grad():
        ...
        for j in range(10):
            ...
            logger.debug(f"image {j} pos inpainted ")
        ...
        for j in range(10):
            ...
            logger.debug(f"image {j} neg inpainted ")
...
def run_recon_current_model(image_in,image_out, config, state):
    
    detector = Detector(config, None, state)
    mask_info_dict = get_mask_info_dict()

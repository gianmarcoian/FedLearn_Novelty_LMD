...
def get_mask(mask_type, 
    ...
    if mask_type == "random":
        ...
        return torch.tensor(maskgen()).unsqueeze(0).repeat(num_channels, 1, 1)
    
    elif mask_type == "center":
        mask = torch.ones((num_channels, image_size, image_size))
...
def get_checkerboard_mask(image_size, num_blocks, num_channels=3, inverted=False):
    assert image_size % num_blocks == 0, 'image_size must be divisible by num_blocks'
    block_len = int(image_size // num_blocks)
    mask = torch.ones((num_channels, num_blocks, num_blocks))
    if inverted:
        mask[:, ::2, ::2] = 0.
        mask[:, 1::2, 1::2] = 0.
    else:
        mask[:,1::2, ::2] = 0.
        mask[:,::2, 1::2] = 0.
    return mask.repeat_interleave(repeats=block_len,dim=1).repeat_interleave(repeats=block_len,dim=2)
